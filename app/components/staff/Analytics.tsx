    ...
    <div>
      ...
      <div className="p-6 px-8">
        <div className="grid grid-cols-4 gap-3.5 mb-5">
          ...
          <MetricCard label="Avg Engagement" value={`${Math.round(students.reduce((a, s) => a + s.engagement, 0) / students.length)}%`} color="#d97706" />
          <MetricCard label="Avg GPA" value={(students.reduce((a, s) => a + s.gpa, 0) / students.length).toFixed(2)} color="#7c3aed" />
        </div>

        <Card noPadding style={{ overflow: "hidden" }}>
          <div className="px-6 py-3 border-b border-line" style={{ background: "#f8f9fb" }}>
            <span className="text-base font-bold text-heading">Student Engagement</span>
          </div>
          {/* Header */}
          <div className="grid px-6 py-2.5 border-b border-line" style={{ gridTemplateColumns: "2fr 1fr 3fr 1fr 1fr" }}>
            {["Student", "Grade", "Engagement", "Schools", "Overdue"].map((h) => (
          ...
          {students.map((s) => (
            <div key={s.id} onClick={() => { onSelectStudent(s); onNavigate("detail"); }}
              ...
              style={{ gridTemplateColumns: "2fr 1fr 3fr 1fr 1fr" }}>
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 h-1.5 rounded-sm" style={{ background: "#eef0f4" }}>
                  <div className="h-full rounded-sm" style={{ width: `${s.engagement}%`, background: s.engagement > 80 ? "#16a34a" : s.engagement > 60 ? "#d97706" : "#ef4444" }} />
                </div>
              ...
              <span className="text-sm font-semibold" style={{ color: s.dl.filter((d) => d.status === "overdue").length > 0 ? "#ef4444" : "#16a34a" }}>
