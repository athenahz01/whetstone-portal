...
export function StudentDetail({ student: s, onBack, onRefresh, profileId }: StudentDetailProps) {
  ...
  return (
    <div>
      <PageHeader
        title={s.name}
        sub={`${(s as any).studentType === "graduate" ? "Graduate Student" : `Grade ${s.grade === 13 ? "PG" : s.grade}`} · ${s.school} · Class of ${s.gradYear}${s.email ? ` · ${s.email}` : ""}`}
        right={
          <div className="flex gap-2 items-center">
            {(s as any).studentType === "graduate" && (
              <span className="text-[10px] font-bold px-2.5 py-1 rounded-full" style={{ background: "rgba(164,128,242,0.1)", color: "#a480f2", border: "1px solid rgba(164,128,242,0.2)" }}>🎓 Graduate</span>
            )}
            <button
      ...
      <div className="p-5 px-6">
        {/* ── Metric Cards ─────────────────────────────────────────────── */}
        <div className={`grid gap-3 mb-5`} style={{ gridTemplateColumns: (s as any).studentType === "graduate" ? "repeat(3, 1fr)" : "repeat(5, 1fr)" }}>
          {(s as any).studentType !== "graduate" && (
            <MetricCard
              label="Schools"
        ...
        <div className="grid grid-cols-2 gap-3.5">
          ...
          <Card>
            ...
            {s.dl.filter((d: any) => d.status !== "completed" && !d.studentOnly).length === 0 && (
              <p className="text-sm text-sub py-4 text-center">No active tasks</p>
            )}
            {s.dl.filter((d: any) => d.status !== "completed" && !d.studentOnly).sort((a, b) => a.days - b.days).map((d) => {
          ...
          {/* Schools — hidden for graduate students */}
          {(s as any).studentType !== "graduate" && (
          <Card>
            <div className="flex justify-between items-center mb-3.5">
